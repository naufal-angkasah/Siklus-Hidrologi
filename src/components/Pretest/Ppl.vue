            ...
            <div class="radio-list" style="font-size: 10px">
              ...
              <div v-for="(ppl, index) in ekoliterasi" :key="ppl.id" class="question-content">
                <div v-show="index === questionIndex" class="question question-box">
                  <p style="font-size: 20px">{{ index + 1 }}. {{ ppl.pernyataan }}</p>
                  <ul>
                    <li  class="radio-item form-check py-2 " v-for="response in ppl.opsi" :key="response.id">
                      <input @click="emitSelectedOption(index + 1, response.id)" :id="response.id + ppl.pernyataan"
                        ...
                        <label style="font-size: 20px" class="form-check-label justify-content-start" :for="response.id + ppl.pernyataan">{{ response.id }}. {{ response.isi }}
                      </label>
                    </li>
                  </ul>
                ...
                ">Selesai &#8250;</CButton>
                    ...
                    <div v-else class="row d-flex justify-content-sm-end pt-4">
                      <button class="btn btn-lg btn-info" style ="font-size:20px" :class="choose ? '' : 'disabled'" v-on:click="next">
                        Berikutnya &#8250;
                      </button>
                    </div>
                  </div>
...
var ppl = [
  ...
  },
  {
    id: 5,
    pernyataan:
      'Saya berusaha memelihara kran air agar tidak cepat rusak dan segera menggantinya bila rusak/ bocor. ',
    opsi: [
      {
        id: 'a',
        isi: 'Selalu',
...
export default {
  ...
  methods: {
    async addData() {
     ...
     await axios
        .post(`https://api-kakerol-git-main-fathinafifs-projects.vercel.app/api/v1/jawabanPPL/`, datas)
        .then((response) => console.log("berhasil:",response.data))
        .catch((error) =>{return console.log('Gagal : ', error)})
