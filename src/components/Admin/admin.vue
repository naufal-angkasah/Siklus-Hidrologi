        ...
        <b-row>
          ...
          <b-col col lg="2" align-self="end">
            <b-dropdown
              ...
              class="col-6"
            >
              <b-dropdown-item href="#" @click="exportExcelPretestKognitif"
                >Kognitif</b-dropdown-item
              >
              <b-dropdown-item href="#" @click="exportExcelPretestEkoliterasi"
                >Ekoliterasi</b-dropdown-item
              >
              <b-dropdown-item href="#" @click="exportExcelPretestSpritual"
                >Spritual</b-dropdown-item
              >
            </b-dropdown>
            ...
            <b-dropdown
              ...
              class="col-6 px-2"
            >
              <b-dropdown-item href="#" @click="exportExcelPosttestKognitif"
                >Kognitif</b-dropdown-item
              >
              <b-dropdown-item href="#" @click="exportExcelPosttestEkoliterasi"
                >Ekoliterasi</b-dropdown-item
              >
              <b-dropdown-item href="#" @click="exportExcelPosttestSpritual"
                >Spritual</b-dropdown-item
              >
            </b-dropdown>
          </b-col>
...
export default {
  ...
  methods: {
    ...
    setKognitif(data) {
      this.jawabanKognitif = data.kognitif
    },
    setSpritual(data) {
      this.jawabanSpritual = data.spritual
    },
    setPPL(data) {
      this.jawabanPPL = data.ppl
    },
    searchId(data, id) {
    ...
    exportExcelPosttestKognitif() {
      ...
      for (let i = 0; i < this.jawabanKognitif.length; i++) {
        ...
        if (this.jawabanKognitif[i].isKognitifPosttest === true) {
          ...
          for (
            let j = 0;
            j < this.jawabanKognitif[i].jawabanKognitif.length;
            j++
          ) {
            let num =
              this.jawabanKognitif[i].jawabanKognitif[j][0][0].toString()
    ...
    exportExcelPretestEkoliterasi() {
      ...
      for (let i = 0; i < this.jawabanPPL.length ; i++) {
        ...
        if( user[0] === undefined) continue
        // console.log( "user :", user[0])
        //jika sudah pretest kognitif
        if (this.jawabanPPL[i].isPPL === true) {
          // console.log("id :", user[0].id,"nama :", user[0].name,", jawaban PPL: ",this.jawabanPPL[i].jawabanPPL)
          // console.log( "nama :", user[0].name)
          data.push({ nama_siswa:  user[0].name})
          for (let j = 0; j < this.jawabanPPL[i].jawabanPPL.length; j++) {
            ...
            //  console.log( "num :", num)
             //tambahkan jawbaan pada setiap soal
            data[idx][num] = this.jawabanPPL[i].jawabanPPL[j][0][1]
            // console.log( "data :", data[idx][num])
          }
          // data[idx]["nama_siswa"] = user[0].name
          idx++
          // no++
    ...
    exportExcelPosttestSpritual() {
      ...
      for (let i = 0; i < this.jawabanSpritual.length; i++) {
        ...
        if (this.jawabanSpritual[i].isSpritualPosttest === true) {
          ...
          for (
            let j = 0;
            j < this.jawabanSpritual[i].jawabanSpritual.length;
            j++
          ) {
            ...
            data[idx][num] =
              this.jawabanSpritual[i].jawabanSpritual[j][1].toString()
          }
          // console.log(data)
          idx++
          // no++
  ...
  mounted() {
    ...
    axios
      .get(`https://api-kakerol-git-main-fathinafifs-projects.vercel.app/api/v1/user`)
      .then((response) => {
        $('#myTable').DataTable({
          // dom: "Bfrtip",
    ...
    axios
      .get(`https://api-kakerol-git-main-fathinafifs-projects.vercel.app/api/v1/jawabanKognitif`)
      .then((response) => this.setKognitif(response.data))
      .catch((error) => console.log('Gagal : ', error))
    axios
      .get(`https://api-kakerol-git-main-fathinafifs-projects.vercel.app/api/v1/jawabanSpritual`)
      .then((response) => this.setSpritual(response.data))
      .catch((error) => console.log('Gagal : ', error))
    axios
      .get(`https://api-kakerol-git-main-fathinafifs-projects.vercel.app/api/v1/jawabanPPL`)
